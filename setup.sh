...
step4_iptables() {
    header "步骤 4/5：配置 iptables 端口转发"
...
注意：
  - smbd 必须在 Termux App 中启动，不能通过 ADB/su 启动
